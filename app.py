...
ticker_input = st.sidebar.text_input("Digite o Ticker (ex: BBAS3, ITUB4, PETR4)", "").upper().strip()
...
def carregar_dados_completos(t):
    ...
    try:
        ...
        # Fiscalização Matemática Anti-Distorção
...
# 4. LOGICA PRINCIPAL
if ticker_input:
    with st.spinner("Sincronizando dados e indicadores de mercado..."):
        # Processamento em lote para garantir sincronia do CDI/IPCA
    ...
    if df_acao is not None:
        ...
        if not df_v.empty:
            ...
            # Benchmarks (Hover arredondado para evitar dízimas)
            if not s_cdi.empty:
            ...
            def calcular_tudo(df_full, valor_mensal, anos, s_cdi_f, s_ipca_f, s_ibov_f):
                ...
                # Cálculo Ativo com Reinvestimento
                ...
                with col:
                    if vf > 0:
                        ...
                        <div class="info-card">
                            ...
                            <div class="card-item">📈 <b>Ibovespa (Média Bolsa):</b> {formata_br(v_ibov)}</div>
                            <div class="card-item">🛡️ <b>Reposição IPCA (Poder Compra):</b> {formata_br(v_ipca)}</div>
                            <div class="card-header" style="margin-top:15px;">📊 Análise da Carteira</div>
            ...
            <div class="glossario-item">
                📌 <b>Poder de Compra (IPCA):</b> Este valor é a sua "linha de empate" contra a inflação. Ele mostra quanto dinheiro você precisaria ter hoje para manter o mesmo padrão de vida que tinha ao fazer cada aporte no passado. Como os preços eram mais baixos antigamente, o valor corrigido hoje reflete o esforço real de poupança.
            </div>
            <div class="glossario-item">
                📌 <b>Proventos (Div/JCP):</b> Inclui Dividendos e Juros Sobre Capital Próprio. O simulador assume que toda distribuição em dinheiro foi imediatamente reinvestida em novas ações da própria empresa, potencializando o efeito dos juros compostos.
            </div>
            <div class="glossario-item">
                📌 <b>Cálculo Blindado:</b> O algoritmo monitora variações bruscas de preço para isolar o impacto de <b>desdobramentos (splits), grupamentos e bonificações</b>, garantindo que o lucro exibido seja real e não um erro estatístico.
            </div>
            </div>
            """, unsafe_allow_html=True)
            
    else: st.error("Ticker não encontrado ou sem dados para o período.")
else: st.info("💡 Digite um Ticker no menu lateral para iniciar a simulação.")
