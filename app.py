...
if ticker_input:
    ...
    if df_acao is not None:
        ...
        # Gráfico principal respeitando o filtro de datas
        if not df_v.empty:
            df_v["Total_Fact_Chart"] = df_v["Total_Fact"] / df_v["Total_Fact"].iloc[0]
            df_v["Price_Base_Chart"] = df_v["Close"] / df_v["Close"].iloc[0]
            
            fig = go.Figure()
            # Benchmarks do gráfico (carregados sob demanda para o período do gráfico)
            if mostrar_cdi:
            ...
            fig.update_layout(template="plotly_white", hovermode="x unified", yaxis=dict(side="right", ticksuffix="%", tickformat=".0f"), margin=dict(l=10, r=10, t=40, b=10))
            ...
            # NOVA FUNÇÃO DE CÁLCULO DINÂMICO
            def calcular_janela_movel(df_full, v_aporte, anos, d_fim_alvo, d_inicio_limite):
                # Define a data de início da janela (ex: Data Fim - 10 anos)
                d_inicio_janela = pd.to_datetime(d_fim_alvo) - pd.DateOffset(years=anos)
                
                # Validação: Se a janela começar antes da data de Início selecionada pelo usuário ou antes do histórico do ticker
                if d_inicio_janela < pd.to_datetime(d_inicio_limite) or d_inicio_janela < df_full.index[0]:
                    return None
                
                # Filtra os dados apenas dessa janela específica
                ...
                # Cálculo de Patrimônio na Janela
                ...
                # Benchmarks para a janela específica
                ...
                return {
                    ...
                    "cdi": calc_ref(s_cdi), "ipca": calc_ref(s_ipca)
                }
            ...
            for anos, col in [(10, col1), (5, col2), (1, col3)]:
                ...
                with col:
                    if res:
                        ...
                        st.markdown(f'<div class="total-card"><div class="total-label">{titulo_col}</div><div class="aviso-periodo">Período fora do intervalo selecionado</div></div>', unsafe_allow_html=True)
                        st.markdown('<div class="info-card"><div class="aviso-periodo">Aumente o intervalo de datas no menu lateral para habilitar esta simulação.</div></div>', unsafe_allow_html=True)

            st.markdown("""<div class="glossario-container">...</div>""", unsafe_allow_html=True) # Glossário mantido igual
