...
def carregar_dados_completos(t):
    ...
    try:
        # Download robusto para garantir Adj Close e evitar erro de 'Ticker não encontrado'
        ...
        # FISCALIZAÇÃO MATEMÁTICA ANTI-ERRO (Proteção CSMG3)
        df["Ret_Total"] = df["Adj Close"].pct_change().fillna(0)
        df["Ret_Preco"] = df["Close"].pct_change().fillna(0)
        # Yield Real Extraído: Ignora variações de preço nominais (como quedas por split)
...
if ticker_input:
    ...
    if df_acao is not None:
        ...
        if not df_v.empty:
            ...
            # --- CORREÇÃO DE SYNC: Carregamos índices de referência PRIMEIRO ---
            if mostrar_cdi:
            ...
            # 5. CARDS DE PATRIMÔNIO (Lucro Acumulado Total)
            ...
            def calcular_patrimonio(df_full, valor_mensal, anos):
                ...
                # Quantidade de cotas compradas pelo preço nominal (tela)
                total_cotas = sum(valor_mensal / df_full.loc[d, 'Close'] for d in datas_aporte)
                # Reinvestimento calculado pelo fator blindado de retorno total
            ...
            • <b>CDI (Certificado de Depósito Interbancário):</b> Referência de Renda Fixa. Caminha próximo à Selic. Se a ação rende menos que o CDI, o risco de bolsa não compensou o retorno seguro.<br><br>
            ...
            • <b>Ibovespa (Mercado):</b> Média das ações mais negociadas. Serve para validar se sua escolha individual performou melhor ou pior que o mercado geral.
            </div>
            """, unsafe_allow_html=True)
